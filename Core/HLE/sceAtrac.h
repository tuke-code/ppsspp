...
struct SceAtracIdInfo {
    ...
    char numFrame; // 20  // This seems to just trigger for a single frame when looping. "numFrame" seems to be the wrong name.
    ...
    u32_le dataEnd; // 36
    s32_le loopNum; // 40
    u32_le streamDataByte; // 44  // Amount of queued data. In full and half-way modes, this isn't decremented per block.
